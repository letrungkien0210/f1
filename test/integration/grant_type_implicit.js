...
describe('Grant Type Implicit', () => {
  it('should redirect when trying to get authorization without logging in', () =>
    ...
    .then(() => {
      return helper.getAuthorization({ responseType: 'token' });
    })
    .then(res => {
      expect(res.request.href.indexOf('/#access_token='))
        .to
        .eql(-1);
    }));
    // get(properties.logout)
    //   .then(() => helper.getAuthorization({ responseType: 'token' }))
    //   .then(([response]) => expect(response.request.href.indexOf('/#access_token='))
    //     .to
    //     .eql(-1)));
  ...
  it('should work with the implicit asking for a access token', () =>
    helper.login()
      ...
      .then(response => validate.userJson(response, response.body)));
  
  it('should give an error with an invalid client id', () =>
    helper.login()
      .then(() => helper.getAuthorization({
        responseType: 'token',
        clientId: 'someinvalidclientid'
      }))
      .catch(error => {
  ...
  it('should give an error with a missing client id', () =>
    helper.login()
      .then(() => {
        return rp({
          method: 'GET',
          uri: `${properties.authorization}?redirect_uri=${properties.redirect}&response_type=token`,
          strictSSL: false,
  ...
  it('should give an error with an invalid response type', () =>
    helper.login()
      ...
      .then(response => expect(response.statusCode)
        .to
        .eql(501))
      .catch(error => expect(error.statusCode)
        .to
        .eql(501))
  );
});
